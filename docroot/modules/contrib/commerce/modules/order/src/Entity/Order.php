  ...
  public function collectAdjustments() {
    ...
    foreach ($this->getItems() as $order_item) {
      foreach ($order_item->getAdjustments() as $adjustment) {
        ...
        if (!empty($this->get('coupons')->target_id) && $adjustment->getType() != 'cypress_cart_rules'
          && $adjustment->getType() != 'cypress_promocode' && $adjustment->getType() != 'cypress_promocode_quantity') {
         ...
         // get the product id from respective promotion
          ...
          if ($offer instanceof ProductQuantityFixedOff) {
            ...
            if ($order_unit_price >= $amount) {
              if ($order_item->getQuantity() <= $coupon_quantity) {
                $price = ($order_item->getQuantity() * ($amount));
                $price = (string) $price;
              }
              else {
                $price = (($coupon_quantity) * ($amount));
                $price = (string) $price;
              }
              $multiplied_adjustment = new Adjustment([
                'type' => $adjustment->getType(),
            ...
            elseif ($order_unit_price < $amount) {
              if ($order_item->getQuantity() <= $coupon_quantity) {
                $price = (($order_unit_price)) * $order_item->getQuantity();
                $price = (string) $price;
              }
              elseif($order_item->getQuantity() > $coupon_quantity) {
                $price = (($coupon_quantity) * ($order_unit_price));
                $price = (string) $price;
              }
              $multiplied_adjustment = new Adjustment([
                'type' => $adjustment->getType(),
