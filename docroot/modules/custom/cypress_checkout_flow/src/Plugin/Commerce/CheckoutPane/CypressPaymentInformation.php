...
/**
 * Provides the payment information pane for cypress.
 *
 * Absolute copy of PaymentInformation checkout pane with modification.
 *
 * @CommerceCheckoutPane(
 *   id = "cypress_payment_information",
 *   label = @Translation("Payment information"),
 *   default_step = "order_information",
 *   wrapper_element = "fieldset",
 * )
 */
class CypressPaymentInformation extends BillingInformationPaneBase {
  ...
  protected function buildPaymentMethodOptions(array $payment_gateways) {
    ...
    // $payment_method_storage = $this->entityTypeManager->getStorage
    // 'commerce_payment_method');
    // foreach ($payment_gateways_with_payment_methods as $payment_gateway_id
    // => $payment_gateway) {
    // $payment_methods = $payment_method_storage->loadReusable($customer,
    // $payment_gateway, $billing_countries);
  ...
  /**
   * Helper function to get billing profile.
   */
  protected function getBillingProfile() {
    ...
    foreach ($this->order->shipments->referencedEntities() as $shipment) {
      $billing_profile = $shipment->getShippingProfile();
      break;
    }
